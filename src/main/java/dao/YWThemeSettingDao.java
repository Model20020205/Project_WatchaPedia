...
public class YWThemeSettingDao {
	 public void createMenu(String name, int storeIdx) throws Exception {
	    ...
	    public List<YWMenuListDto> getMenuList(int muIdx) throws Exception {
	        Connection conn = YWJdbcCommon.getConnection();
	        ...
	        List<YWMenuListDto> menuList = new ArrayList<>();
	        while (rs.next()) {
	            String name = rs.getString("name");
	            String link = rs.getString("link");
	            int upperMul = rs.getInt("upper_mul");
	            menuList.add(new YWMenuListDto(name, link, upperMul));
	        }
